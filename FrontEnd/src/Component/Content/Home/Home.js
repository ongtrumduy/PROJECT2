...
import preleft from "../../Image-Icon/Arrow Left.png";
import nextright from "../../Image-Icon/Arrow Right.png";
...
export default class Home extends React.Component {
    ...
    return (
      <div>
        ...
        <div className="unknow-friend-infor-render">
          <p>
            {" "}
            <img alt="option" list src={list} /> Họ và tên:{" "}
            {this.state.lastname} {this.state.firstname}
          </p>
          <p>
            {" "}
            <img alt="option" list src={list} /> Ngày sinh: {this.state.birth}
          </p>
          <p>
            {" "}
            <img alt="option" src={list} /> Giới tính: {this.state.gender}
          </p>
          <p>
            {" "}
            <img alt="option" src={list} /> Sở thích: {this.state.enjoy}
          </p>
        </div>
      </div>
    );
  ...
  unknowFriendControl = () => {
    return (
      <div className="unknow-friend-button-icon ">
        <div>
          <button onClick={() => this.PreUnknowUser()}>
            <img alt="pre" src={preleft} />
          </button>
        </div>
        <div>{this.userProfileIcon()}</div>
        <div>
          <button onClick={() => this.NextUnknowUser()}>
            {" "}
            <img alt="next" src={nextright} />
          </button>
        </div>
      </div>
    );
  ...
  renderUnknowUser = () => {
    if (this.state.statusunknowuser === 0) {
      return <div>HIỆN TẠI TRONG ỨNG DỤNG CHỈ CÓ BẠN VÀ ADMIN :))))</div>;
    } else {
      return (
  ...
  render() {
    return <div>{this.renderUnknowUser()}</div>;
  }
}
