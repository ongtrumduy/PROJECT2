...
import destroy from "../../Image-Icon/Stop 2.png";
...
export default class UserTotal extends React.Component {
  ...
  destroyUserFromList = (_adminid, _userid) => {
    let confirm = window.confirm(
      "Xác nhận khóa tài khoản của người dùng này???"
    );
    if (confirm) {
      let adminuser = {
        adminid: _adminid,
          ...
          <table>
            ...
            <tbody>
              {this.state.totaluserlist.map((item, index) => (
                <tr key={index}>
                  <td>{this.getUserList(item.username)}</td>
                  <td>{this.getUserList(item.friendname)}</td>
                  <td>
                    <div className="user-total-friend-select">
                      <button
                        onClick={() =>
                          this.destroyUserFromList(
                            this.props.userid,
                            item.userid
                          )
                        }
  ...
  render() {
    return (
      <div className="user-total">
        ...
        <div>{this.headTotalUserList()}</div>
        <div className="user-total-body">{this.renderUserTotalList()}</div>
      </div>
    );
  }
}
